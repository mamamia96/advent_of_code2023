...
def check_neighbors(x, y, data):
    # check if any neighbors are parts (symbols)
    checks = [
        (-1, -1), (-1, 0), (-1, 1),
        (1, -1), (1, 0), (1, 1),
        (0, -1), (0, 1)
    ...
    not_right = '.1234567890'
    
    if x == 0:
        checks = [pt for pt in checks if pt[0] != -1]
    elif x == len(data[0]) - 1:
        checks = [pt for pt in checks if pt[0] != 1]
        
    if y == 0:
        checks = [pt for pt in checks if pt[1] != -1]
    elif y == len(data) - 1:
        checks = [pt for pt in checks if pt[1] != 1]
        
    for check in checks:
        if data[y + check[1]][x + check[0]] not in not_right:
            return True, (y + check[1], x + check[0], data[y + check[1]][x + check[0]])
...
def main():
    ...
    for i in range(len(lines)):
        ...
        for j in range(len(lines[i])):
            if lines[i][j] in str_nums:
                part_id += lines[i][j]
                if not valid_part:
                    valid_part, part_tuple = check_neighbors(j, i, lines)
                    print(part_tuple)
                if j == len(lines[i]) - 1 and valid_part:
                    parts.append(int(part_id))
                    if part_tuple[2] != '*': continue
                    my_key = str(part_tuple[0]) + ':' + str(part_tuple[1])
                    if not my_key in parts_dict:
                        parts_dict[my_key] = [int(part_id)]
                    else:
                        parts_dict[my_key].append((int(part_id)))
                    part_id = ""
                    valid_part = False
            else:
                if valid_part:
    ...
    for key,val in parts_dict.items():
        print(val)
        if len(val) == 2: part_two_sum += (val[0] * val[1])
    
    print(f'valid parts sum {sum(parts)}')
    print(f'gear sums {part_two_sum}')
    
main_input = """...766.......821.547.....577......................................387.....................56..........446.793..........292..................
