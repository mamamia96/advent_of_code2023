...
def check_neighbors(x, y, data):
    ...
    for check in checks:
        if data[y + check[1]][x + check[0]] == '*':
            return (y + check[1], x + check[0])
...
def main():
    ...
    for i in range(len(lines)):
        ...
        for j in range(len(lines[i])):
            if lines[i][j] in str_nums:
                part_id += lines[i][j]
                if not valid_part:
                    gear_pos = check_neighbors(j, i, lines)
                    if gear_pos != (-1, -1): valid_part = True
                if j == len(lines[i]) - 1 and valid_part:
                    if gear_pos not in parts_dict:
                        parts_dict[gear_pos] = [int(part_id)]
                    else:
                        parts_dict[gear_pos].append(int(part_id))
    ...
    # check results from our dictionary
    for key, val in parts_dict.items():
        if len(val) == 2:
            part_two_sum += (val[0] * val[1])
